...
class BbiiMessage extends CActiveRecord
{
	...
	public function mailboxFull($attr, $params) {
		...
		$criteria->condition = 'sendto = '. Yii::app()->user->id;
		if(BbiiMessage::model()->outbox()->count($criteria) >=50) {
			$this->addError('sendto', Yii::t('bbii', 'Your outbox is full. Please make room before sending new messages.'));
		}
	}

	public function blocked($attribute, $params) {
		if(BbiiIpaddress::blocked($this->ip)) {
			$this->addError('ip', Yii::t('bbii','Your IP address has been blocked.'));
		}
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'sendfrom' => Yii::t('bbii', 'From'),
			'sendto' => Yii::t('bbii', 'To'),
			'subject' => Yii::t('bbii', 'Subject'),
			'content' => Yii::t('bbii', 'Content'),
			'create_time' => Yii::t('bbii', 'Posted'),
			'read_indicator' => Yii::t('bbii', 'Read'),
			'type' => Yii::t('bbii', 'Type'),
			'inbox' => Yii::t('bbii', 'Inbox'),
			'outbox' => Yii::t('bbii', 'Outbox'),
			'ip' => 'Ip',
		);
	}
