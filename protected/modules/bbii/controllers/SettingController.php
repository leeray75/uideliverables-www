...
class SettingController extends BbiiController {
	...
	public function actionDeleteForum() {
		...
		if(isset($_POST['id'])) {
			...
			if(BbiiForum::model()->exists("cat_id = " . $_POST['id'])) {
				$json['success'] = 'no';
				$json['message'] = Yii::t('bbii', 'There are still forums in this category. Remove these before deleting the category.');
			} elseif(BbiiTopic::model()->exists('forum_id = ' . $_POST['id'])) {
				$json['success'] = 'no';
				$json['message'] = Yii::t('bbii', 'There are still topics in this forum. Remove these before deleting the forum.');
			} else {
	...
	public function actionDeleteMembergroup() {
		...
		if(isset($_POST['id'])) {
			if($_POST['id'] == 0) {
				$json['success'] = 'no';
				$json['message'] = Yii::t('bbii', 'The default member group cannot be removed.');
			} else {
